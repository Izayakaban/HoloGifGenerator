...
async function grabGifFromApi(query) {
  ...
  const response = await axios.get(`https://api.giphy.com/v1/gifs/search?q=${encodeURIComponent(query)}&api_key=${giphyCpiKey}&limit=10`);
...
async function generateGifs(query) {

//apparently there is a limit of 5 gifs per row, but I hate limits so I added this line in case I feel like making it more than 5
  query.preventDefault();
  display.innerHTML = "";
  const inputData = getInputData();
  const gifs = await grabGifFromApi('Hololive ' + inputData);
